...
namespace WalkGame.Tests
{
    ...
    public sealed class M88SaveMigratorContractTests
    {
        ...
        {
            // With Current=1, zero is unsupported and fails fast. This test proves
            // the migrator does not infinite-loop or break-and-succeed.
            var profile = new PlayerProfile { schemaVersion = 0 };
            ...
            Assert.IsNotNull(error);
            // Error should indicate unsupported or no migration path, but never empty.
            Assert.IsTrue(error.Length > 5);
        }
